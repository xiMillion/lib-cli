...
const googleTr = (text,target) => {
    const _RegExp = new RegExp('[\\u4E00-\\u9FFF]','g');
    const source = _RegExp.test(text) ? 'zh-CN' : 'en';

    const _URL =
  `https://translate.google.cn/translate_a/single?client=webapp&sl=${source}&tl=${target}&hl=${source}&dt=at&dt=bd&dt=ex&dt=ld&dt=md&dt=qca&dt=rw&dt=rm&dt=sos&dt=ss&dt=t&otf=1&ssel=3&tsel=3&xid=45662847&kc=1`;
...
// googleTr('hello','zh-CN').then((d)=>{
//     console.log(d)
// })

module.exports = googleTr;